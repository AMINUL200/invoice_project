// pages/purchase/HandlePurchaseOrder.jsx
...
const HandlePurchaseOrder = () => {
  ...
  const fetchPurchaseOrders = useCallback(async (page = 1) => {
    ...
    try {
      const response = await api.get(`/org/purchase-orders?page=${page}`);
      ...
      setApiError(error.message || "Failed to fetch purchase orders");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "subtotal" || name === "tax_amount") {
      const subtotal =
        parseFloat(name === "subtotal" ? value : formData.subtotal) || 0;
      const tax =
        parseFloat(name === "tax_amount" ? value : formData.tax_amount) || 0;
  ...
  const validateForm = () => {
    ...
    if (!formData.po_no?.trim()) {
      errors.po_no = "PO number is required";
    }
  ...
  const handleEditPO = (po) => {
    ...
    setFormData({
      ...
      subtotal: parseFloat(po.subtotal).toString(),
      tax_amount: parseFloat(po.tax_amount).toString(),
      total_amount: parseFloat(po.total_amount).toString(),
      status: po.status,
    });
    setFormErrors({});
    setApiError(null);
    ...
    try {
      ...
      const poData = {
        ...
        total_amount:
          ...
          parseFloat(formData.subtotal) + parseFloat(formData.tax_amount),
      ...
      if (editingPO) {
        ...
        response = await api.put(
          `/org/purchase-orders/${editingPO.id}`,
          poData,
        );
        setApiSuccess("Purchase order updated successfully");
      } else {
        // Add new PO
        response = await api.post("/org/purchase-orders", poData);
        setApiSuccess("Purchase order created successfully");
      }
      ...
      if (error.data?.errors) {
        ...
        setApiError(error.message || "Failed to save purchase order");
      }
    } finally {
  ...
  const handleDeletePO = async (poId) => {
    ...
    try {
      await api.delete(`/org/purchase-orders/${poId}`);
      setApiSuccess("Purchase order deleted successfully");
      fetchPurchaseOrders(pagination.current_page);
    } catch (error) {
  ...
  const handleStatusChange = async (poId, newStatus) => {
    try {
      await api.patch(`/org/purchase-orders/${poId}/status`, {
        status: newStatus,
      });
      fetchPurchaseOrders(pagination.current_page);
      setApiSuccess(`Purchase order status updated to ${newStatus}`);
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      draft: { bg: "bg-[#E2E8F0]", text: "text-[#475569]", icon: Clock },
      sent: { bg: "bg-[#DBEAFE]", text: "text-[#1D4ED8]", icon: Send },
      confirmed: {
        bg: "bg-[#DCFCE7]",
        text: "text-[#15803D]",
        icon: CheckCircle,
      },
      cancelled: { bg: "bg-[#FEE2E2]", text: "text-[#B91C1C]", icon: XCircle },
    };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[#64748B] mt-1">
                Manage your purchase orders and track vendor payments
              </p>
            </div>
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-[#334155] mb-1">
                        PO Number <span className="text-[#EF4444]">*</span>
                      </label>
                      <input
                        type="text"
                        name="po_no"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-[#334155] mb-1">
                        PO Date <span className="text-[#EF4444]">*</span>
                      </label>
                      <input
                        type="date"
                        name="po_date"
...
export default HandlePurchaseOrder;
